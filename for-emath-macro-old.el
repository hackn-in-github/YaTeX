...
(defun YaTeX:YniSessen ()
  (let* ((option (YaTeX-emath-setoption "" "infx" "計算範囲のxの下限"))
	 (option (YaTeX-emath-setoption option "supx" "計算範囲のxの上限"))
	 (option (YaTeX-emath-setoption option "xval" "接点のx座標を受け取る制御綴り"))
	 (option (YaTeX-emath-setoption option "lim" "近似の精度の調整[値を小さくすると精度が上がる](.0001)")))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">"))
		    "{" (read-string "関数: ") "}"
		    "{" (read-string "導関数: ") "}"
...
(defun YaTeX:KTkukan ()
  (let* ((emnum 0)
    ...
    (progn (insert (concat (if (y-or-n-p "ラベルの設定をしますか？: ")
			       (concat "[" (YaTeX-emath-tenretu-loop "," "ラベルの指定") "]") "")
			   "{" (YaTeX-emath-tenretu-loop ";" "区間の設定") "}"))
	   (setq color (YaTeX-emath-setoption "" "color" "数直線上の解答部分の配色")
		 emnum (YaTeX-emath-get-number-from-list "[option] 共通部分の塗り方" `("塗り潰し" "斜塗り"))
		 paintoption (concat "paintoption={"
				     (cond ((= emnum 1)(concat "<" (emath-emPaint-option "") ">"))
					   ((= emnum 2)(concat "*<" (emath-emPaint*-option "") ">"))
					   (t ""))
				     "}"))
	   (setq option (YaTeX-emath-option-combine "," `(,color ,paintoption)))
	   (insert (concat (if (> (length option) 0)
			       (concat "[" option "]") ""))
		   "{" (read-string "解答の区間: ") "}"))))
...
(defun YaTeX:Kaiten ()
  (let* ((emnum (YaTeX-emath-get-number-from-list "[option] 長さの指定" `("長さ" "倍率" "指定しない")))
...
(defun emath-itiziketugou-loop (seq)
  (let* ((vector "")
	...
	(progn (setq vector (read-string "ベクトルの指定: ")
		     vector (YaTeX-emath-option-combine "," `(,keisuu ,vector))
		     vector (YaTeX-emath-option-combine ";" `(,seq ,vector)))
...
(defun YaTeX:itiziketugou ()
  (let* ((option "")
	 ...
	 (perl (if (y-or-n-p "Perlと連携しますか？")
		   "perl" "")))
    (progn (setq option (YaTeX-emath-option-combine "," `(,iii ,perl)))
	   (if (> (length option) 0)
	       (insert (concat "<" option ">")) "")
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;(defun YaTeX-emath-nuritubusi-option ()
;;;  (let* ((option "")
;;;	 (emnum 0))
;;;    (progn (if (y-or-n-p "塗り潰しの指定をしますか？: ")
;;;	       (setq emnum (YaTeX-emath-get-number-from-list "[option] 塗り潰しの指定" `("白黒指定" "カラー指定"))) "")
;;;	   (setq option (cond ((= emnum 1)(concat "noudo=" (read-string "[option] 濃度指定(.5): ")))
;;;			      ((= emnum 2)(concat "paintcolor=" (read-string "[option] 色指定: ")))
;;;			      (t "")))
;;;	   (if (= emnum 2)
;;;	       (setq option (YaTeX-emath-setoption option "dilutecolor" "塗り色の濃度指定[0で段階的に薄くする]")) "")
;;;	   (concat option))))
;;;(defun YaTeX-emath-syanuri-option ()
;;;  (let* ((temp (read-string "[option] 斜線の方向角(45): "))
;;;	 (angle (if (string= temp "") ""
;;;		  (concat "[" temp "]")))
;;;	 (option (YaTeX-emath-setoption "" "border" "1で境界を表示(1)"))
;;;	 (option (YaTeX-emath-setoption option "bordercolor" "境界の色"))
;;;	 (option (YaTeX-emath-setoption option "syanurisiteiten" "斜線群が通過する点の指定"))
;;;	 (option (YaTeX-emath-setoption option "syanurikankaku" "斜線の間隔(.125)"))
;;;	 (option (YaTeX-emath-setoption option "syanuriKankaku" "斜線の間隔(要単位)"))
;;;	 (option (YaTeX-emath-setoption option "slashcolor" "斜線の色"))
;;;	 (option (YaTeX-emath-setoption option "syasentanmatu" "斜線の端と囲む線との間隔(要単位)")))
;;;    (concat angle
;;;	    (if (string= option "") ""
;;;	      (concat "<" option ">")))))
(defun YaTeX-emath-En-body ()
  (let* ((emnum 0)
	 (hankei ""))
    (concat "{" (read-string "円の中心: ") "}"
	    "{" (setq emnum (YaTeX-emath-get-number-from-list "半径の指定方法" `("半径指定" "通過点指定"))
...
(defun YaTeX:En ()
  (let* ((option (YaTeX-emath-drawline-option)))
    (concat (if (string= option "") ""
	    ...
	    (YaTeX-emath-En-body))))
(defun YaTeX:En* ()
  (let* ((option (emath-emPaint-paintonly-option));(YaTeX-emath-nuritubusi-option))
	 (border (if (y-or-n-p "円周を描画しますか？: ")
		     "border" ""))
	 (color (if (> (length border) 0)
		    (concat "color=" (read-string "[option] 円周の色の指定: ")) "")))
    (progn (setq option (YaTeX-emath-option-combine "," `(,option ,border ,color)))
...
(defun YaTeX-emath-Enko-option ()
  (let* ((emnum 0)
	 (arc (if (y-or-n-p "描画方向を逆にしますか？: ") "arcn" ""))
	 (option (YaTeX-emath-setoption "" "hamidasikaku" "はみ出し角"))
	 (option (YaTeX-emath-setoption option "hazimeT" "始点の名前変更(hazimeT)"))
	 (option (YaTeX-emath-setoption option "owariT" "終点の名前変更(owariT)"))
	 (yazirusi ""))
    (progn (if (y-or-n-p "矢印を付加しますけ？: ")
	       (setq emnum (YaTeX-emath-get-number-from-list "[option] 矢印の位置の指定" `("正の向き" "負の向き" "両端"))
		     yazirusi (concat "yazirusi="
				      (cond ((= emnum 1) (concat "a"))
					    ((= emnum 2) (concat "r"))
					    ((= emnum 3) (concat "b"))))) "")
	   (setq option (YaTeX-emath-option-combine "," `(,arc ,option ,yazirusi)))
	   (concat option))))
(defun YaTeX-emath-Enko-body ()
  (concat "{"
	  (let* ((empoint (YaTeX-emath-setoption "" "tuukaten" "[半径] 円の通過点")))
	    (if (string= empoint "")
		(read-string "円の半径: ")
	      (concat empoint)))
	  "}{"
	  (let* ((empoint (YaTeX-emath-setoption "" "hazimeten" "[開始角] 開始点")))
	    (if (string= empoint "")
		(read-string "開始角: ")
	      (concat empoint)))
	  "}{"
	  (let* ((empoint (YaTeX-emath-setoption "" "owariten" "[終了角] 終了点")))
	    (if (string= empoint "")
		(read-string "終了角: ")
	      (concat empoint)))
	  "}"))
(defun YaTeX:Enko ()
  (let* ((option "")
	 (lineop (YaTeX-emath-drawline-option))
	 (enkoop (YaTeX-emath-Enko-option)))
    (progn (setq option (YaTeX-emath-option-combine "," `(,lineop ,enkoop)))
	   (if (> (length option) 0)
	       (insert (concat "<" option ">")) "")
...
(defun emath-EnkoToubun-option ()
  (let* ((emnum 0)
	 ...
	 (nahuda (if (y-or-n-p "[option] 各頂点に名札を設定しますか？: ")
		     "nahuda" ""))
	 (option (YaTeX-emath-setoption "" "oresen" "点列を保存するの制御綴り名")))
    (progn (if (y-or-n-p "[option] 折線を描画しますか？: ")
	       (setq emnum (YaTeX-emath-get-number-from-list "[option] 描画方法"
							     `("\\Drawline" "\\Takakkei" "\\emPaint" "\\emPaint*"))
		     dousa (cond ((= emnum 1)
				  (concat "dousa={D"
					  (if (y-or-n-p "[option] \\Drawlineのオプションを設定しますか？: ")
					      (concat "<" (YaTeX-emath-Drawline-option) ">") "")
					  "}"))
				 ((= emnum 2)
				  (concat "dousa={T"
				 ...
				 ((= emnum 4)
				  (concat "dousa={P*"
					  ...
					  "}"))
				 (t ""))))
	   (setq option (YaTeX-emath-option-combine "," `(,option ,kuromaru ,nahuda ,dousa)))
	   (concat option))))
...
(defun YaTeX:ArrowArc ()
  (let* ((option (YaTeX-emath-Enko-option)))
    (concat (if (string= option "") ""
	      (concat "<" option ">"))
	    "{" (read-string "半径: ") "}{"
	    (read-string "始点: ") "}{"
...
;	  (YaTeX-emath-syanuri-option)
	  (YaTeX-emath-Daen-body)))
(defun YaTeX-emath-Daenko-body ()
  (concat (YaTeX-emath-Daen-body)
	  "{" (read-string "開始角: ") "}{"
	  (read-string "終了角: ") "}"))
(defun YaTeX:Daenko ()
  (let* ((option (YaTeX-emath-drawline-option))
	 (option (YaTeX-emath-setoption option "yazirusi" "矢印の指定[arbn](n)")))
    (concat (if (string= option "") ""
	      (concat "<" option ">"))
	    (YaTeX-emath-Daenko-body))))
(defun YaTeX:Daenko* ()
  (let* ((option (emath-emPaint-paintonly-option)));(YaTeX-emath-nuritubusi-option)))
    (concat (if (string= option "") ""
	      (concat "[" option "]"))
...
(defun YaTeX:LandL ()
  (concat (read-string "直線1の通る点1: ")
	  (read-string "直線1の通る点2: ")
	  (read-string "直線2の通る点1: ")
	  (read-string "直線2の通る点2: ")
	  (read-string "2直線の交点: ")))
;\Landng<#1>#2#3#4#5#6
;    #1: key=val
;    2点 #2, #3 を通る直線と
...
(defun emath-Lline-option ()
  (let* ((option (YaTeX-emath-setoption "" "linethickness" "直線の太さを指定"))
	 (dash (emath-set-dash))
	 (option (YaTeX-emath-setoption option "color" "直線の色の指定"))
	 (hidariM (if (y-or-n-p "左端点の文字列を指定しますか？: ")
		      (concat "hidariM={" (YaTeX-emath-Put-HouiShitei)
			      "{" (read-string "[option] [左端点] 文字列の指定: ") "}}") ""))
	 (migiM (if (y-or-n-p "右端点の文字列を指定しますか？: ")
		    (concat "migiM={" (YaTeX-emath-Put-HouiShitei)
			    "{" (read-string "[option] [右端点] 文字列の指定: ") "}}") ""))
	 (tanten (emath-set-tanten-csname))
;	 (option (YaTeX-emath-setoption option "hidariT" "左端点の名前を指定"))
;	 (option (YaTeX-emath-setoption option "migiT" "右端点の名前を指定"))
	 (option (YaTeX-emath-setoption option "func" "Perl形式の直線の式を保存する制御綴り"))
	 (option (YaTeX-emath-option-combine "," `(,option ,dash ,hidariM ,migiM ,tanten))))
    (concat option)))
(defun YaTeX:Lline ()
...
(defun YaTeX:gline ()
  (let* ((option (emath-Lline-option))
	 (perl (if (y-or-n-p "傾きにPerlの計算式を許容しますか？: ")
		   "perl" ""))
	 (option (YaTeX-emath-option-combine "," `(,option ,perl))))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
		    "{" (read-string "通過点: ") "}"
		    "{" (read-string "傾き: ") "}%"))))
...
(defun emath-Hline-common ()
  (let* ((option (YaTeX-emath-setoption "" "linethickness" "半直線の太さ"))
	 (option (YaTeX-emath-setoption option "color" "半直線の色"))
	 (dash (emath-set-dash))
	 (option (YaTeX-emath-setoption option "HlineT" "描画端点を表す制御綴り"))
	 (option (YaTeX-emath-option-combine "," `(,option ,dash))))
    (concat option)))
(defun YaTeX:LHline ()
...
(defun YaTeX:kaitenkigou ()
  (let* ((option (YaTeX-emath-setoption "" "linethickness" "楕円の太さ"))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 矢印の向き"
						  `("正の回転方向(defalut)" "負の回転方向" "両方向")))
	 (yazirusi (cond ((= emnum 2) "yazirusi=r")
			 ((= emnum 3) "yazirusi=b")
			 (t "")))
	 (option (YaTeX-emath-setoption option "bairitu" "倍率(1)"))
	 (option (YaTeX-emath-setoption option "hazimekaku" "開始角(15)"))
	 (option (YaTeX-emath-setoption option "owarikaku" "終了角(345)"))
	 (option (YaTeX-emath-setoption option "tyouhankei" "長半径(3mm)"))
	 (option (YaTeX-emath-setoption option "tanhankei" "短半径(1.5mm)"))
...
(defun emath-kakukigou-common (str)
  (let* ((emnum 0)
	 (op1num (YaTeX-emath-get-number-from-list "[option] 円弧修飾" 
    ...
    (progn (insert (concat op1 op2
			   (if (string= str "loop")
			       (concat "{" (YaTeX-emath-tenretu-loop ";" "角を表す点列") "}")
			     (read-string "角を表す点列: "))))
	   (setq option (YaTeX-emath-setoption "" "linethickness" "弧の太さ")
		 dash (emath-set-dash)
		 emnum (YaTeX-emath-get-number-from-list "[option] 円弧の描画法" `("円(default)" "bezier"))
		 bezier (if (= emnum 2) (YaTeX-emath-setoption "" "bezier" "頂点から円弧中央に対する比の設定: ") "")
		 option (YaTeX-emath-setoption option "Hankei" "円弧の半径指定")
		 bounagasa (if (= op1num 4) (YaTeX-emath-setoption "" "Kakukigoubounagasa" "縦棒の長さ指定") "")
		 kankaku (if (> enkonum 1)
			     (YaTeX-emath-setoption "" "Kakukigoukankaku" "円弧の間隔指定") "")
		 option (YaTeX-emath-setoption option "moziiti" "文字列配置位置の設定(10pt)")
		 kakukigou (if (string= str "loop")
			       (YaTeX-emath-setoption "" "kakukigou" "各内に配置する記号の設定") "")
		 option (YaTeX-emath-option-combine "," `(,option ,dash ,bezier ,bounagasa ,kankaku ,kakukigou)))
	   (insert (if (> (length option) 0)
		       (concat "<" option ">") "")))))
(defun YaTeX:Kakukigou ()
  (progn (concat (emath-kakukigou-common "")
		 (YaTeX-emath-Put-HouiShitei)
...
(defun YaTeX:ippankaku ()
  (let* ((option (YaTeX-emath-setoption "" "hazimekaku" "始め角の指定"))
	 (option (YaTeX-emath-setoption option "linethickness" "螺旋の太さ"))
	 (option (YaTeX-emath-setoption option "color" "螺旋の色"))
	 (emnum (YaTeX-emath-get-number-from-list "[option] r=aθ+bの係数aの設定"
	 ...
	 (rasenA (cond ((= emnum 1) (concat "rasenA=" (read-string "[option] aに対する倍率: ")))
		       ...
		       (t "")))
	 (emnum (YaTeX-emath-get-number-from-list "[option] r=aθ+bの係数bの設定"
	 ...
	 (rasenB (cond ((= emnum 1) (concat "rasenB=" (read-string "[option] bに対する倍率: ")))
		       ...
		       (t "")))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 矢印の設定"
						  `("終点のみ(default)" "始点のみ" "両端" "なし")))
	 (yazirusi (cond ((= emnum 2) "yazirusi=r")
			 ((= emnum 3) "yazirusi=b")
			 ((= emnum 4) "yazirusi=n")
			 (t "")))
	 (option (YaTeX-emath-option-combine "," `(,option ,rasenA ,rasenB ,yazirusi))))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
		    "{" (read-string "一般角指定: ") "}%"))))
...
(defun emath-tyokkakukigou-option ()
  (let* ((option (YaTeX-emath-setoption "" "size" "記号のサイズの指定(5pt)"))
	 (option (YaTeX-emath-setoption option "linethickness" "線の太さの指定"))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 記号の回転" 
						  `("回転させない" "90度回転" "180度回転" "-90度回転")))
	 (kaiten (if (and (> emnum 1) (> 4 emnum))
		     (concat "kaiten=" (number-to-string (* 90 (- emnum 1)))) ""))
	 (option (YaTeX-emath-option-combine "," `(,option ,kaiten))))
    (concat option)))
...
(defun YaTeX:tyokkakukigou ()
  (let* ((option (emath-tyokkakukigou-option)))
    (insert (concat (if (> (length option) 0)
			("<" option ">") "")
		    "{" (YaTeX-emath-tenretu-loop ";" "直角を作る点列[3点]") "}%"))))
(fset `YaTeX:perpmark `YaTeX:Tyokkakukigou)
(defun YaTeX:heikoukigou ()
  (let* ((option (YaTeX-emath-setoption "" "heikoukigoukosuu" "記号の個数(1)"))
	 (option (YaTeX-emath-setoption option "heikoukigouiti" "記号の位置(.5)"))
	 (option (YaTeX-emath-setoption option "heikoukigoukankaku" "記号の間隔(1mm)"))
	 (option (YaTeX-emath-setoption option "heikoukigousize" "記号の大きさ(2)")))
    (concat (if (string= option "") ""
	      (concat "[" option "]"))
	    "{" (YaTeX-emath-tenretu-loop ";" "記号を置く線分の2端点") "}%")))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;%     #5, #6 : 線分の両端                                       ;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;(defun YaTeX-emath-touhenkigou-option ()
...
;;;  (concat (YaTeX-emath-touhenkigou-option)
;;;	  "{" (YaTeX-emath-tenretu-loop ";" "線分の2端点") "}"))
;;;(defun YaTeX:Touhenkigou ()
;;;  (concat (YaTeX-emath-touhenkigou-option)
;;;	  (read-string "線分の端点1: ")
;;;	  (read-string "線分の端点2: ")))
...
(defun YaTeX:rotUbrace ()
  (concat (if (y-or-n-p "線分と\\underbraceの距離を調整しますか?")
	  ...
	  "{" (read-string "文字列: ") "}%"))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; YaTeX-emath-tenretu-input-elements ()                                 ;;;
;;; \tenretu などの [label]Name(x.y)[pos].. を繰り返し入力させる          ;;;
;;;   YaTeX-emath-tenretu-input-elements "" "x" "y"              ;;;
...
(defun YaTeX-emath-tenretu-input-elements (var1 var2 &optional ast-or-not coord seq)
  (let* ((emlabel (concat (if ast-or-not ""
    ...
    (let* ((option (concat (if (string= emlabel "") ""
	   ...
	   (emseq (if seq seq ""))
	   (option (YaTeX-emath-option-combine ";" `(,emseq ,option))))
      (if (y-or-n-p "続けますか？")
	  (YaTeX-emath-tenretu-input-elements var1 var2 ast-or-not coord option)
	(concat "{" option "}")))))
(defun emath-tenretu-option ()
  (let* ((op1 (if (y-or-n-p "[option] Perlと連携しますか？: ")
	 ...
	 (kuromaru (if (y-or-n-p "[option] 設定した点に黒丸を配置しますか？: ")
		       "kuromaru" ""))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 点の結び方の設定"
						 `("折線" "多角形" "塗り潰し" "斜線" "設定しない")))
	 (dousa (cond ((= emnum 1) (concat "dousa={D<" (YaTeX-emath-drawline-option) ">}"))
		      ((= emnum 2) (concat "dousa={T<" (YaTeX-emath-drawline-option) ">}"))
		      ((= emnum 3) (concat "dousa={P<" (emath-emPaint-option "") ">}"))
		      ((= emnum 4) (concat "dousa={P*<" (emath-emPaint*-slashonly-option) ">}"))
		      (t "")))
	 (op1 (YaTeX-emath-setoption op1 "oresen" "点列を保存する制御綴り名"))
	 (op1 (YaTeX-emath-option-combine "," `(,op1 ,kuromaru ,dousa)))
...
(defun YaTeX:tenretu* ()
  (let* ((option (emath-tenretu-option))
	 ...
	 (op2 (car (cdr option))))
    (insert (concat op1 op2
		    (YaTeX-emath-tenretu-input-elements "x" "y" t)))))
(defun YaTeX:tenretu** ()
  (let* ((option (if (y-or-n-p "各点に黒丸を設定しますか？: ")
		       "kuromaru" ""))
	 (showlabel (if (y-or-n-p "名札を非表示にしますか？: ")
			"showlabel=0" ""))
	 (option (YaTeX-emath-option-combine "," `(,option ,showlabel))))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
		    (YaTeX-emath-tenretu-input-elements "x" "y" nil t)))))
(defun YaTeX:rtenretu ()
  (let* ((option (emath-tenretu-option))
	 (op1 (car option))
	 (op2 (car (cdr option))))
    (insert (concat op1 op2
		    (YaTeX-emath-tenretu-input-elements "r" "θ")))))
(defun YaTeX:rtenretu* ()
  (let* ((option (emath-tenretu-option))
	 (op1 (car option))
	 (op2 (car (cdr option))))
    (insert (concat op1 op2
		    (YaTeX-emath-tenretu-input-elements "r" "θ" t)))))
;\emRbezier<#1>#2#3#4
;    #1: key=val
...
(defun emath-addvec-common ()
  (let* ((emnum (YaTeX-emath-get-number-from-list "[option] 方向と長さの指定" `("直交座標" "極座標")))
...
(defun emath-PutStr-option ()
  (let* ((emnum (YaTeX-emath-get-number-from-list "[option] 弧のサイズ指定"
	 ...
	 (arrowsize (if (y-or-n-p "矢印の大きさを指定しますか？: ")
			(concat "arrowsize={" (YaTeX-emath-setarrow ) "}") ""))
	 (linethickness (YaTeX-emath-setoption "" "linethickness" "線の太さ"))
	 (option (YaTeX-emath-option-combine "," `(,hankei ,addvec ,arrowsize ,linethickness))))
    (concat option)))
(defun YaTeX:PutStr ()
  (let* ((option ""))
  ...
  (let* ((option ""))
    (progn (insert (concat "{" (YaTeX:read-coordinates "終点からの変位ベクトルの指定 (x,y): " "x" "y") "}"
	   ...
	   (insert (concat (if (> (length option) 0)
			   ...
			   "{" (read-string "矢線の終点: ") "}")))))
(defun YaTeX:PutStr ()
  (let* ((empos (YaTeX-emath-Put-HouiShitei))
	 (option (YaTeX-emath-setoption "" "enkoH" "線分の中点と弧の中点との距離(要単位)"))
	 (option (YaTeX-emath-setoption option "Hankei" "弧の半径"))
	 (option (YaTeX-emath-setoption option "arrowheadsize" "鏃の倍率"))
	 (option (YaTeX-emath-setoption option "addvec" "矢印の先端の微調整 (x,y) or r(r,θ)" "{" "}")))
...
(defun YaTeX:PutStr* ()
  (let* ((empos (YaTeX-emath-Put-HouiShitei))
	 (option (YaTeX-emath-setoption "" "enkoH" "線分の中点と弧の中点との距離(要単位)"))
	 (option (YaTeX-emath-setoption option "hankei" "弧の半径(無名数)"))
	 (option (YaTeX-emath-setoption option "Hankei" "弧の半径(要単位)"))
	 (option (YaTeX-emath-setoption option "arrowheadsize" "鏃の倍率"))
	 (option (YaTeX-emath-setoption option "addvec" "矢印の先端の微調整 (x,y) or r(r,θ)" "{" "}")))
...
(defun emath-Takakkei-common ()
  (let* ((option (YaTeX-emath-Drawline-option))
	 (option (YaTeX-emath-setoption option "boutyou" "多角形の拡大縮小[要単位]"))
	 ...
	 (setxyrange (if (y-or-n-p "多角形を内包する長方形の大きさを書き出しますか？: ")
			 "setxyrange" ""))
	 (option (YaTeX-emath-option-combine "," `(,option ,paintcolor ,setxyrange))))
    (concat option)))
...
(defun YaTeX:Takakkeis ()
  (let* ((option (emath-Takakkei-common)))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
		    "{" (YaTeX-emath-tenretu-loop ";" "多角形を作る点列: ") "}%"))))
...
(defun YaTeX:kyokuTyoku ()
  (concat (YaTeX:read-coordinates "(r,θ) r:単位付き数値 θ:60分法")
	  ...
	  "%"))
;;;;;(defun YaTeX-emath-zahyou-body (&optional ps-or-not)
;;;;;  (concat (YaTeX:read-coordinates "(xmin,xmax): ")
;;;;;	  (YaTeX:read-coordinates "(ymin,ymax): ")
;;;;;	  "%\n"
;;;;;	  (let* ((emfontsize (YaTeX-emath-get-number-from-list "文字の大きさ" `("\\small" "\\scriptsize" "自分で設定"))))
...
;;;;;(defun YaTeX-emath-zahyou-option ()
;;;;;  (let* ((emsymbol (if (y-or-n-p "[option] 座標の記号を変更しますか？")
;;;;;		       (let* ((temp (YaTeX-emath-setoption "" "gentenkigou" "原点記号(O)"))
;;;;;			      (temp (YaTeX-emath-setoption temp "yokozikukigou" "横軸記号($x$)"))
;;;;;			      (temp (YaTeX-emath-setoption temp "tatezikukigou" "縦軸記号($y$)")))
;;;;;			 (concat temp)) ""))
;;;;;	 (empos (if (y-or-n-p "[option] 座標の配置を変更しますか？")
;;;;;		    (let* ((temp (YaTeX-emath-setoption "" "gentenhaiti" "原点の配置(se)" "{[" "]}"))
;;;;;			   (temp (YaTeX-emath-setoption temp "yokozikuhaiti" "横軸記号の配置" "{[" "]}"))
;;;;;			   (temp (YaTeX-emath-setoption temp "tatezikuhaiti" "縦軸記号の配置" "{[" "]}")))
;;;;;		      (concat temp)) ""))
;;;;;	 (emarrow (if (y-or-n-p "[option] 軸の形状を変えますか？")
;;;;;		      (let* ((temp (YaTeX-emath-setoption "" "arrowheadsize" "鏃の倍率(1)"))
;;;;;			     (temp (YaTeX-emath-setoption temp "zikusensyu" "軸の線種")))
;;;;;			(concat temp)) "")))
;;;;;    (concat (YaTeX-emath-option-combine "," `(,emsymbol ,empos ,emarrow)))))
;;;;;(defun YaTeX-emath-zahyou*-option ()
;;;;;  (let* ((emul (YaTeX-emath-setoption "" "ul" "1目盛りの単位長(要単位)"))
;;;;;	 (emmargin (if (y-or-n-p "[option] 余白の調整をしますか？")
;;;;;		       (let* ((temp (YaTeX-emath-setoption "" "Yohaku" "余白(要単位)"))
;;;;;			      (temp (YaTeX-emath-setoption temp "yohaku" "余白(無名数)"))
;;;;;			      (temp (YaTeX-emath-setoption temp "Migiyohaku" "右余白(要単位)"))
;;;;;			      (temp (YaTeX-emath-setoption temp "migiyohaku" "右余白(無名数)"))
;;;;;			      (temp (YaTeX-emath-setoption temp "Hidariyohaku" "左余白(要単位)"))
;;;;;			      (temp (YaTeX-emath-setoption temp "hidariyohaku" "左余白(無名数)"))
;;;;;			      (temp (YaTeX-emath-setoption temp "Ueyohaku" "上余白(要単位)"))
;;;;;			      (temp (YaTeX-emath-setoption temp "ueyohaku" "上余白(無名数)"))
;;;;;			      (temp (YaTeX-emath-setoption temp "Sitayohaku" "下余白(要単位)"))
;;;;;			      (temp (YaTeX-emath-setoption temp "sitayohaku" "下余白(無名数)")))
;;;;;			 (concat temp)) ""))
;;;;;	 (emscale (if (y-or-n-p "[option] 縦横比を変えますか？")
;;;;;		      (let* ((temp (YaTeX-emath-setoption "" "xscale" "横の比率(1)"))
;;;;;			     (temp (YaTeX-emath-setoption temp "yscale" "縦の比率(1)")))
;;;;;			(concat temp)) ""))
;;;;;	 (empos (if (y-or-n-p "[option] 環境自体の縦の配置を調整しますか？")
;;;;;		    (YaTeX-emath-select-key-value "haiti" "[option]" `(("上揃え" "t") ("下揃え" "b") ("中央揃え" "c") ("x軸揃え" "x")))
;;;;;		  ""))
;;;;;	 (emunit (if (y-or-n-p "[option] 斜交座標にしますか？")
;;;;;		     (let* ((temp (YaTeX-emath-setoption "" "ex" "x軸の単位ベクトル"))
;;;;;			    (temp (YaTeX-emath-setoption temp "ey" "y軸の単位ベクトル")))
;;;;;		       (concat temp)) "")))
;;;;;    (YaTeX-emath-option-combine "," `(,emul ,emmargin ,emscale ,empos ,emunit))))
;;;;;(defun YaTeX-emath-pszahyou-option ()
;;;;;  (let* ((emps (YaTeX-emath-setoption "" "borderwidth" "クリッピング対策で実描画部を広くするマージンの設定"))
;;;;;	 (emps (YaTeX-emath-setoption emps "EPSfilename" "独自のEPSファイル名の設定"))
;;;;;	 (emclip (if (y-or-n-p "[option] クリッピングを止めますか？")
;;;;;		     "EPSclip=false" "")))
;;;;;    (YaTeX-emath-option-combine "," `(,emps ,emclip))))
;;;;;(defun YaTeX:zahyou ()
;;;;;  (let* ((astop (YaTeX-emath-zahyou*-option))
;;;;;	 (zop (YaTeX-emath-zahyou-option))
;;;;;	 (option (YaTeX-emath-option-combine "," `(,astop ,zop))))
;;;;;    (concat (if (string= option "") ""
;;;;;	      (concat "[" option "]"))
;;;;;	    "%\n\t\t"
;;;;;	    (YaTeX-emath-zahyou-body))))
;;;;;(defun YaTeX:zahyou* ()
;;;;;  (let* ((option (YaTeX-emath-zahyou*-option)))
;;;;;    (concat (if (string= option "") ""
;;;;;	      (concat "[" option "]"))
;;;;;	    "%\n\t\t"
;;;;;	    (YaTeX-emath-zahyou-body)
...
;;;;;  (let* ((astop (YaTeX-emath-zahyou*-option))
;;;;;	 (zop (YaTeX-emath-zahyou-option))
;;;;;	 (psop (YaTeX-emath-pszahyou-option))
;;;;;	 (option (YaTeX-emath-option-combine "," `(,astop ,zop ,psop))))
;;;;;    (concat (if (string= option "") ""
;;;;;	      (concat "[" option "]"))
;;;;;	    "%\n\t\t"
;;;;;	    (YaTeX-emath-zahyou-body t)
;;;;;	    "\n%\t\\xmax=\\truexmax+borderwidthのようになっているので\\xmaxなどを使用するときは注意")))
;;;;;(defun YaTeX:pszahyou* ()
;;;;;  (let* ((astop (YaTeX-emath-zahyou*-option))
;;;;;	 (psop (YaTeX-emath-pszahyou-option))
;;;;;	 (option (YaTeX-emath-option-combine "," `(,astop ,psop))))
;;;;;    (concat (if (string= option "") ""
;;;;;	      (concat "[" option "]"))
;;;;;	    "%\n\t\t"
;;;;;	    (YaTeX-emath-zahyou-body t)
...
(defun emath-zahyou-option ()
  (let* ((emnum 0)
	 (option (YaTeX-emath-setoption "" "ul" "単位長の設定[1cm程度を推奨](1pt)"))
	 (option (YaTeX-emath-setoption option "zikusensyu" "縦・横軸の線種指定"))
	 (xscale (YaTeX-emath-setoption "" "xscale" "横軸方向の伸縮率[要emathPxy]"))
	 (yscale (YaTeX-emath-setoption "" "yscale" "縦軸方向の伸縮率[要emathPxy]"))
	 (advice (if (or (> (length xscale) 0) (> (length yscale) 0))
		     "xscale,yscaleオプションはemathPxyが必要" ""))
	 (option (YaTeX-emath-option-combine "," `(,option ,xscale ,yscale)))
	 (kigou "");gentenkigou,yokozikukigou,tatezikukigou
	 (haiti "");gentenhaiti,yokozikuhaiti,tatezikuhaiti
	 (yohaku "");yohaku,ueyohaku,sitayohaku,migiyohaku,hidariyohaku
	 (envhaiti ""))
    (progn (if (y-or-n-p "記号の設定をしますか？: ")
	       (setq kigou (YaTeX-emath-setoption "" "gentenkigou" "原点の記号(O)")
		     kigou (YaTeX-emath-setoption kigou "yokozikukigou" "横軸の記号($x$)")
		     kigou (YaTeX-emath-setoption kigou "tatezikukigou" "縦軸の記号($y$)")) "")
	   (if (y-or-n-p "記号の配置に関する設定をしますか？: ")
	       (setq haiti (YaTeX-emath-setoption "" "gentenhaiti" "原点の位置([sw])" "{" "}")
		     haiti (YaTeX-emath-setoption haiti
						  "yokozikuhaiti" "横軸記号の位置((0,-3pt)[rt])" "{" "}")
		     haiti (YaTeX-emath-setoption haiti
						  "tatezikuhaiti" "縦軸記号の位置((-3pt,0)[rt])" "{" "}")) "")
	   (if (y-or-n-p "余白の設定をしますか？: ")
	       (setq emnum (YaTeX-emath-get-number-from-list "[option] 余白の設定"
							     `("一括設定" "上下左右の個別設定"))
		     yohaku (if (= emnum 1)
				(YaTeX-emath-setoption "" "yohaku" "上下左右の余白") "")
		     yohaku (if (= emnum 2)
				(YaTeX-emath-setoption "" "ueyohaku" "環境上部の余白") yohaku)
		     yohaku (if (= emnum 2)
				(YaTeX-emath-setoption yohaku "sitayohaku" "環境下部の余白") yohaku)
		     yohaku (if (= emnum 2)
				(YaTeX-emath-setoption yohaku "hidariyohaku" "環境左部の余白") yohaku)
		     yohaku (if (= emnum 2)
				(YaTeX-emath-setoption yohaku "migiyohaku" "環境右部の余白") yohaku)) "")
	   (setq emnum (YaTeX-emath-get-number-from-list "[option] 環境のベースラインの設定" `("下辺(default)" "上辺" "x軸"))
		 envhaiti (cond ((= emnum 2) "haiti=t")
				((= emnum 3) "haiti=x")
				(t ""))
		 option (YaTeX-emath-option-combine "," `(,option ,kigou ,haiti ,yohaku ,envhaiti)))
	   (list option advice))))
...
(defun YaTeX:zahyou ()
  (let* ((option (emath-zahyou-option))
	 ...
	 (option (car option))
	 (ex (YaTeX-emath-setoption "" "ex" "横軸の単位ベクトル(要emathPha)" "{" "}"))
	 (ey (YaTeX-emath-setoption "" "ey" "縦軸の単位ベクトル(要emathPha)" "{" "}"))
	 (advice (if (or (> (length ex) 0) (> (length ey) 0))
		     (concat advice (if (> (length advice) 0) " : " "") "ex,eyオプションはemathPhaが必要です")))
	 (option (YaTeX-emath-option-combine "," `(,option ,ex ,ey))))
    (concat "%\n\t" (if (> (length option) 0)
			(concat "[" option "]") "")
	    "%\n\t"
	    (emath-zahyou-common)
...
(defun YaTeX:pszahyou ()
  (let* ((option (emath-zahyou-option))
	 (advice (car (cdr option)))
	 (option (car option))
	 (option (YaTeX-emath-setoption option "borderwidth" "クリッピング対策で実描画部を広くするマージンの設定"))
	 (range (if (y-or-n-p "getxyrangeを設定しますか？: ")
		    "getxyrange" ""))
	 (clip (if (y-or-n-p "クリッピングを止めますか？: ")
		   "EPSclip=false" ""))
	 (option (YaTeX-emath-setoption option "EPSfilename" "独自のEPSファイル名の設定"))
	 (option (YaTeX-emath-option-combine "," `(,option ,range ,clip))))
    (concat "%\n\t" (if (> (length option) 0)
			(concat "[" option "]") "")
	    "%\n\t"
	    (emath-zahyou-common)
...
(defun YaTeX:psazahyou ()
  (let* ((option (emath-zahyou-option))
	 (option (YaTeX-emath-setoption option "ex" "横軸の単位ベクトル" "{" "}"))
	 (option (YaTeX-emath-setoption option "ey" "縦軸の単位ベクトル" "{" "}"))
	 (option (YaTeX-emath-setoption option "borderwidth" "クリッピング対策で実描画部を広くするマージンの設定"))
	 (range (if (y-or-n-p "getxyrangeを設定しますか？: ")
		    "getxyrange" ""))
	 (clip (if (y-or-n-p "クリッピングを止めますか？: ")
    ...
    (concat "%\n\t" (if (> (length option) 0)
	    ...
	    "%\n%\t要emathPsa.sty"
	    "%\n%\t\\xmax=\\truexmax+borderwidthのようになっているので\\xmaxなどを使用するときは注意")))
(fset `YaTeX:psazahyou* `YaTeX:psazahyou)
(defun YaTeX:zahyouMemori ()
  (let* ((emnum (YaTeX-emath-get-number-from-list "[option] 格子点等の指定"
	 ...
	 (emonlygrid (if (y-or-n-p "グリッドのみにしますか？")
			 "[n]" ""))
	 (option (YaTeX-emath-drawline-option))
	 (option (YaTeX-emath-setoption option "dx" "xの刻み値"))
	 (option (YaTeX-emath-setoption option "dy" "yの刻み値"))
	 (option (YaTeX-emath-setoption option "ox" "xの基準値"))
	 (option (YaTeX-emath-setoption option "oy" "yの基準値")))
...
(defun YaTeX:xMemori ()
  (let* ((option (if (y-or-n-p "[option] Perlと連携しますか？")
		     "perl" ""))
	 (option (YaTeX-emath-setoption option "label" "数字以外のものを表示"))
	 (option (YaTeX-emath-setoption option "memorinagasa" "目盛りの長さ"))
	 (temp (YaTeX-emath-Put-HouiShitei))
	 (emhoui (if (string= temp "") ""
		   (concat "memoriiti={" temp "}")))
	 (option (YaTeX-emath-option-combine "," `(,option ,emhoui))))
    (concat (if (string= option "") ""
	      (concat "<" option ">"))
...
(defun YaTeX:csyndiv ()
  (concat "{" (read-string "割られる式の係数(csv形式): ") ";%\n"
	  ...
	  (read-string "代入する数値: ") "}"))
(defun YaTeX:calcval ()
  (let* ((emnum (YaTeX-emath-get-number-from-list "[option] printfの書式設定" `("浮動小数点(default)" "10進整数" "16進整数" "8進整数" "自分で設定")))
	 (option (cond ((= emnum 1) "")
		       ((= emnum 2) "[d]")
		       ((= emnum 3) "[x]")
		       ((= emnum 4) "[o]")
...
;;			     (temp (YaTeX-emath-setoption temp "zikusensyu" "軸の線種")))
;;			(concat temp)) "")))
;;    (concat (YaTeX-emath-option-combine (list "," `(,emsymbol ,empos ,emarrow))))))
(defun YaTeX-emath-Zahyou*-option ()
  (let* ((emul (YaTeX-emath-setoption "" "ul" "1目盛りの単位長(要単位)"))
	 (emunit (if (y-or-n-p "[option] 各軸の基本ベクトルを調整しますか？")
		     (let* ((unitx (YaTeX-emath-setoption "" "Ex" "x軸の単位ベクトル(r(.667,138))"))
			    (unity (YaTeX-emath-setoption "" "Ey" "y軸の単位ベクトル((1,0))"))
			    (unitz (YaTeX-emath-setoption "" "Ez" "z軸の単位ベクトル((0,1))")))
		       (concat (YaTeX-emath-option-combine "," `(,unitx ,unity ,unitz)))) ""))
	 (emmargin (if (y-or-n-p "[option] 余白の調整をしますか？")
		       (let* ((temp (YaTeX-emath-setoption "" "Yohaku" "余白(要単位)"))
			      (temp (YaTeX-emath-setoption temp "yohaku" "余白(無名数)"))
			      (temp (YaTeX-emath-setoption temp "Migiyohaku" "右余白(要単位)"))
	 ...
	 (empos (if (y-or-n-p "[option] 環境自体の縦の配置を調整しますか？")
		    (YaTeX-emath-select-key-value "haiti" "[option]" `(("上揃え" "t") ("下揃え" "b") ("中央揃え" "c") ("x軸揃え" "x"))))))
    (YaTeX-emath-option-combine "," `(,emul ,emunit ,emmargin ,empos))))
(defun YaTeX:Zahyou ()
  (let* ((astop (YaTeX-emath-Zahyou*-option)))
...
;;; YaTeX-emath-Put-HouiShitei () を使用する                              ;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun YaTeX:emathPut ()
  (let* ((option "")
	 ...
	 (xpos "")
	 (ypos ""))
    (if (y-or-n-p "射影を指定しますか？: ")
	(progn (insert (read-string "文字列を置く座標の指定: "))
	       (setq xynum (YaTeX-emath-get-number-from-list "[option] 射影の種類" `("xy" "x" "y"))
		     syaei (cond ((= xynum 1) "syaei=xy")
				 ((= xynum 2) "syaei=x")
				 ((= xynum 3) "syaei=y")
				 (t ""))
		     houi (if (y-or-n-p "配置文字列の位置を設定しますか？: ")
			      (YaTeX:read-position "news") "")
		     houi (if (> (length houi) 0) (concat "houi={" houi "}"))
		     option (YaTeX-emath-setoption "" "linewidth" "線の太さの指定")
		     dash (emath-set-dash)
		     option (YaTeX-emath-setoption option "syaeiiro" "射影で引かれた線分の色の指定")
		     xlabel (if (or (= xynum 1) (= xynum 2))(YaTeX-emath-setoption "" "xlabel" "垂線の足に置く文字列の指定[x]") "")
		     xpos (if (or (= xynum 1) (= xynum 2))(YaTeX-emath-Put-HouiShitei))
		     xpos (if (> (length xpos) 0) (concat "xpos={" xpos "}") "")
		     ylabel (if (or (= xynum 1) (= xynum 3))(YaTeX-emath-setoption "" "ylabel" "垂線の足に置く文字列の指定[y]") "")
		     ypos (if (or (= xynum 1) (= xynum 3))(YaTeX-emath-Put-HouiShitei))
		     ypos (if (> (length ypos) 0) (concat "ypos={" ypos "}") "")
		     option (YaTeX-emath-option-combine "," `(,syaei ,houi ,option ,dash ,xlabel ,ylabel ,xpos ,ypos)))
	       (insert (concat (if (> (length option) 0)
				   (concat "[" option "]"))
			       "{" (read-string "配置文字列の指定: ") "}%")))
      (progn (if (y-or-n-p "配置文字列を回転させますか？: ")
		 (setq emnum (YaTeX-emath-get-number-from-list "[option] 回転方法" `("回転角指定" "方向ベクトル指定" "方向ベクトルの始点と終点を指定"))
		       houi (cond ((= emnum 1)(concat "kaiten=" (read-string "回転角の指定: ")))
				  ((= emnum 2)(concat "houkou={" (read-string "方向ベクトルの指定: ") "}"))
				  ((= emnum 3)(concat "from={" (read-string "始点の指定: ") "},"
						      "to={" (read-string "終点の指定: ") "}")))) "")
	     (insert (concat (if (> (length houi) 0) (concat "[" houi "]") "")
...
(defun YaTeX:YTenPut ()
  (let* ((option (YaTeX-emath-setoption "" "xformat" "x座標のフォーマット指定(s)"))
	 (option (YaTeX-emath-setoption option "yformat" "y座標のフォーマット(f)")))
    (concat (if (string= option "") ""
	      (concat "[" option "]"))
	    "{" (read-string "関数の指定: ") "}{"
...
(defun YaTeX:PutParts ()
  (let* ((emlabel (YaTeX-emath-setoption "" "label" "labelの設定"))
	 (emparts (YaTeX-emath-select-key-value "" "パーツの選択"
...
(defun emath-kuromaru-option ()
  (let* ((option "")
	 (size (YaTeX-emath-setoption "" "size" "黒丸の半径の指定"))
	 (paintcolor (YaTeX-emath-setoption "" "paintcolor" "色の指定")))
    (setq option (YaTeX-emath-option-combine "," `(,size ,paintcolor)))))
(defun YaTeX:Kuromaru ()
  (let* ((option (emath-kuromaru-option)))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
		    "{" (YaTeX-emath-tenretu-loop "" "黒丸を打つ点の指定")
		    "}%\n%\t広域で半径を指定するには\\KuromaruHankeiを利用する\n"
		    "%\tただし，白丸の半径にも影響するので注意\n"))))
(defun emath-siromaru-option ()
  (let* ((option "")
	 (size (YaTeX-emath-setoption "" "size" "白丸の半径の指定"))
	 (linethickness (YaTeX-emath-setoption "" "linethickness" "円周の太さの指定"))
	 (color (YaTeX-emath-setoption "" "color" "円周の色の指定")))
    (setq option (YaTeX-emath-option-combine "," `(,size ,linethickness ,color)))))
(defun YaTeX:Siromaru ()
  (let* ((option (emath-siromaru-option)))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
		    "{" (YaTeX-emath-tenretu-loop "" "白丸を打つ点の指定")
...
(defun emath-kurosankaku-option ()
  (let* ((option "")
	 (size (YaTeX-emath-setoption "" "size" "外接円の半径の指定"))
	 (paintcolor (YaTeX-emath-setoption "" "paintcolor" "色の指定"))
	 (kaiten (YaTeX-emath-setoption "" "kaiten" "回転角の指定")))
    (setq option (YaTeX-emath-option-combine "," `(,size ,paintcolor ,kaiten)))))
(defun YaTeX:Kurosankaku ()
  (let* ((option (emath-kurosankaku-option)))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
		    "{" (YaTeX-emath-tenretu-loop "" "黒三角を打つ点の指定")
		    "}%\n%\t広域で外接円の半径を指定するには\\KurosankakuHankeiを利用する\n"))))
(defun YaTeX:Kurosikaku ()
  (let* ((option (emath-kurosankaku-option)))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
		    "{" (YaTeX-emath-tenretu-loop "" "黒四角を打つ点の指定")
...
(defun emath-sirosankaku-option ()
  (let* ((option "")
	 (size (YaTeX-emath-setoption "" "size" "外接円の半径の指定"))
	 (linethickness (YaTeX-emath-setoption "" "linethickness" "周の太さの指定"))
	 (color (YaTeX-emath-setoption "" "color" "周の色の指定"))
	 (kaiten (YaTeX-emath-setoption "" "kaiten" "回転角の指定")))
    (setq option (YaTeX-emath-option-combine "," `(,size ,linethickness ,color ,kaiten)))))
(defun YaTeX:Sirosankaku ()
  (let* ((option (emath-sirosankaku-option)))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
		    "{" (YaTeX-emath-tenretu-loop "" "白三角を打つ点の指定")
		    "}%\n%\t広域で外接円の半径を指定するには\\SirosankakuHankeiを利用する\n"))))
(defun YaTeX:Sirosikaku ()
  (let* ((option (emath-sirosankaku-option)))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
		    "{" (YaTeX-emath-tenretu-loop "" "白四角を打つ点の指定")
...
(defun emath-heikoukigou-common ()
  (let* ((option (YaTeX-emath-setoption "" "heikoukigouiti" "記号を置く位置指定[0-1](.5)"))
	 (option (YaTeX-emath-setoption option "heikoukigoukosuu" "矢印の個数指定[1-4](1)"))
	 (option (YaTeX-emath-setoption option "heikoukigoukankaku" "矢印の間隔指定[要単位](1mm)"))
	 (option (YaTeX-emath-setoption option "heikoukigousize" "矢印の大きさ指定(2)"))
	 (option (YaTeX-emath-setoption option "linethickness" "矢印の太さ指定(.4pt)")))
    (concat option)))
(defun YaTeX:Heikoukigou ()
...
(defun YaTeX:heikoukigou ()
  (let* ((option (emath-heikoukigou-common)))
    (insert (concat (if (> (length option) 0)
			(concat "[" option "]") "")
		    "{" (YaTeX-emath-tenretu-loop ";" "線分の指定") "}"))))
(fset 'YaTeX:heikoukigou* 'YaTeX:heikoukigou)
;\HenKo[#1]<#2>#3#4#5
...
(defun emath-HenKo-option ()
  (let* ((option (YaTeX-emath-setoption "" "linewidth" "弧の太さ"))
	 (dash (emath-set-dash));dash & hasenLG
	 (option (YaTeX-emath-setoption option "henkocolor" "弧の色"))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 矢印の種類"
						  `("終点のみ" "始点のみ" "両端" "設定しない")))
	 (yazirusi (cond ((= emnum 1) "yazirusi=a")
			 ((= emnum 2) "yazirusi=r")
			 ((= emnum 3) "yazirusi=b")
			 (t "")))
	 (option (YaTeX-emath-setoption option "henkoH" "弧の頂点と弦の距離(1.6ex)"))
	 (option (YaTeX-emath-setoption option "henkomoziH" "文字列と弦の距離"))
	 (option (YaTeX-emath-setoption option "putpos" "文字列の配置位置[0=端点1，1端点2](.5)"))
	 (putoption (if (y-or-n-p "[option] 文字列の配置位置を調整しますか？: ")
			(concat "putoption={"
				(YaTeX-emath-Put-HouiShitei)
				"}")))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 文字列の回転"
						  `("始点が先頭" "終点が先頭" "設定しない")))
	 (henkomozikaiten (cond ((= emnum 1) "henkomozikaiten=-1")
				((= emnum 2) "henkomozikaiten=1")
				(t "")))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 文字列の背景"
						  `("白抜き(default)" "白抜きをしない")))
	 ...
	 (emnum (YaTeX-emath-get-number-from-list "[option] 端点の垂直方向への移動"
	 ...
	 (agezoko (cond ((= emnum 1) (concat "agezoko=" (read-string "[option] 両端の移動量[要単位]: ")))
			...
			(t "")))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 端点の水平方向への移動"
						  `("両端を同じだけ移動" "端点1・端点2それぞれ移動" "設定しない")))
	 (Hamidasi (cond ((= emnum 1) (concat "Hamidasi=" (read-string "[optoin] 両端の移動量[要単位]: ")))
			 ...
			 (t "")))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 弧の形状"
						  `("楕円" "折線[三角形]" "平行線" "大括弧"
						    "折線[等脚台形]" "小括弧" "中括弧")))
	 ...
	 (option (YaTeX-emath-option-combine "," `(,option ,dash ,yazirusi ,putoption,henkomozikaiten
...
(defun YaTeX:HenKos ()
  (let* ((op1 (read-string "[optoin] 点の個数[*で一任，0で弧を描画しない]: "))
    ...
    (insert (concat (if (> (length op1) 0)
		    ...
		    (if (> (length op2) 0)
			(concat "<" op2 ">") "")
		    "{" (YaTeX-emath-tenretu-loop ";" "2つの端点と配置文字列の指定") "}"))))
;\HenToubun<#1>#2#3#4#5
; 辺#2#3の(#4)等分点を定義します。
;    #1: key=val, 有効なキーは
...
(defun YaTeX:HenToubun ()
  (let* ((emnum 0)
    ...
    (progn (if (y-or-n-p "名札を設定しますか？: ")
		 ...
		 (setq nahuda "nahuda")) "")
	   (setq emnum (YaTeX-emath-get-number-from-list "[option] 等分点の印"
							 `("黒丸" "縦棒" "印を付けない"))
		 shirushi (cond ((= emnum 1)(concat "kuromaru"
						    (if (y-or-n-p "両端にも配置しますか？: ") "=-1" "")))
				((= emnum 2)(concat "tatebou"))
				(t ""))
		 option (YaTeX-emath-option-combine "," `(,nahuda ,shirushi)))
	   (insert (concat (if (> (length option) 0)
			       (concat "<" option ">") "")
			   "{" (read-string "端点1: ") "}"
			   "{" (read-string "端点2: ") "}"
...
(defun emath-set-tanten-csname ()
  (let* ((option (YaTeX-emath-setoption "" "hidariT" "左端点の制御綴り名(\\hidariT)"))
	 (option (YaTeX-emath-setoption option "migiT" "右端点の制御綴り名(\\migiT)")))
    (concat option)))
(defun emath-hamidasisenbun-option ()
  (let* ((op1 (emath-set-tanten-csname))
	 (op2 (YaTeX-emath-Drawline-option)))
    (list op1 op2)))
...
(defun YaTeX:Fsyndiv ()
  (let* ((emsoroe (if (y-or-n-p "[option] 右揃えにしますか？")
	 ...
	 (emsityuu (if (y-or-n-p "[option] 支柱を立てますか？")
		       (YaTeX-emath-select-key-value "" "[option] [支柱]"
						     `(("答え行のみ" "kotaegyousityuu=\\bsityuu")
						       ("すべての行" "gyousityuu=\\bsityuu")))
 ""))
	 (emop (YaTeX-emath-option-combine "," `(,emsoroe ,emsityuu))))
    (concat (if (string= emop "") ""
	      (concat "<" emop ">"))
...
(defun YaTeX:EnGurafu ()
  (let* ((emop (YaTeX-emath-setoption "" "Hankei" "円グラフの半径(2cm)"))
	 (emop (YaTeX-emath-setoption emop "fboxsep" "ラベルの白抜き余白(\\fboxsep)"))
	 (emop (YaTeX-emath-setoption emop "EnGurafuLabelpos"
  ...
  (let* ((emitem (YaTeX-emath-setoption "" "item" "見出しの設定" "{" "}"))
	 ...
	 (emoval (if (string= emoval "") ""
		   (if (y-or-n-p "[option] [oval] 角の四分円の半径を設定しますか？")
		       (YaTeX-emath-setoption "" "rectboxoval" "[oval] 角の四分円の半径(10pt)")
		     "rectboxoval")))
	 ...
	 (emoct (if (string= emoct "") ""
		   (if (y-or-n-p "[option] [oct] 角を切り取る辺の長さを設定しますか？")
		       (YaTeX-emath-setoption "" "rectboxoct" "[oct] 切り取る辺の長さ)")
		     "rectboxoct")))
	 (emop (YaTeX-emath-setoption "" "fboxsep"
				      "余白の設定(環境内の\\fboxsepを利用するアイテムに影響を与える)"))
	 (emop (YaTeX-emath-setoption emop "hsep" "左右の余白の設定"))
	 (emop (YaTeX-emath-setoption emop "vsep" "上下の余白の設定"))
	 (emop (YaTeX-emath-setoption emop "rectboxwidth" "環境の横幅の指定(\\linewidth)"))
	 (emop (YaTeX-emath-setoption emop "linewidth" "枠線の太さの指定"))
	 (emdash (if (y-or-n-p "枠線を破線にしますか？")
		     (let* ((temp (YaTeX-emath-setdash)))
		       (concat "dash={" temp "}")) ""))
	 (emop (YaTeX-emath-option-combine "," `(,emop ,emdash)))
	 (emop (YaTeX-emath-setoption emop "framecolor" "枠線の色の指定"))
	 (emop (YaTeX-emath-setoption emop "backgroundcolor" "背景色の指定"))
	 (emdis (YaTeX-emath-select-key-value "" "[option] 枠線を"
					      `(("左右のみ表示" "LRonly") ("左のみ表示" "Lonly")
						("右のみ表示" "Ronly"))))
	 (emop (YaTeX-emath-option-combine "," `(,emop ,emitem ,emitempos ,embitem
...
;;					  (concat "\\setdash"
;;						  (YaTeX-emath-setdash)) "")))
;;		       (YaTeX-emath-option-combine (list "," templine tempdash))) ""))
)
    (concat (if (string= emop "") ""
	      (concat "[" emop "]%\n"))
;;	    (if (string= emline "") ""
;;	      (concat "<" emline ">"))
		...
		(let* ((emsize (YaTeX-emath-select-key-value "" "文字サイズの変更"
...
(defun YaTeX:EMcallperl ()
  (let* ((emop (YaTeX-emath-setoption "" "scriptfile" "スクリプトファイル名の設定"))
	 (emop (YaTeX-emath-setoption emop "outputfile" "アウトプットファイル名の設定")))
    (concat (if (string= emop "") ""
	      (concat "[" emop "]%")))))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; YaTeX-emath-Drawline-option                                      ;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun YaTeX-emath-Drawline-option ()
  (let* ((option (YaTeX-emath-setoption "" "color" "色の指定"))
	 (option (YaTeX-emath-setoption option "linethickness" "線の太さ"))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 破線の設定"
						  `("hasenLG" "dash" "設定しない")))
	 ...
	 (emnum (YaTeX-emath-get-number-from-list "[option] コーナーの補正"
						  `("マイター(default)" "ラウンド"
						    "ベベル" "オーバル")))
	 (linejoin (cond ((and (> emnum 1) (< emnum 4))
			 ...
			 ((= emnum 4) "oval")
			 (t "")))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 折れ点のマークの種類"
						  `("黒丸" "白丸" "任意" "設定しない")))
	 (mark (cond ((= emnum 1) "kuromaru")
		     ...
		     (t "")))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 端点の処理"
						  `("断ち切り型(default)" "丸型" "角型")))
	 (linecap (cond ((or (= emnum 2) (= emnum 3))
			 (concat "linecap=" (number-to-string (- emnum 1))))
			(t "")))
	 (option (YaTeX-emath-option-combine "," `(,option ,dash ,yazirusi ,arrowsize
							   ,idou ,linejoin ,mark ,linecap))))
    (concat option)))
(defun YaTeX:Drawline ()
  (let* ((emop (YaTeX-emath-Drawline-option)))
    (concat (if (string= emop "") ""
	      (concat "<" emop ">"))
...
(defun YaTeX:Drawlines ()
  (let* ((emop (YaTeX-emath-Drawline-option)))
    (concat (if (string= emop "") ""
	      (concat "<" emop ">"))
	    "{" (YaTeX-emath-tenretu-loop ";" "折れ線を作る点列") "}%")))
(defun YaTeX:Bousin ()
  (concat (read-string "頂点1: ")
	  (read-string "頂点2: ")
	  (read-string "頂点3: ")
...
;;; Henvii:終点をずらす変移ベクトル                ;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun YaTeX-emath-ArrowLine-option ()
  (let* ((allinethickness (YaTeX-emath-setoption "" "allinethickness" "線分の太さ"))
	 (arrowsize (if (y-or-n-p "[option] 鏃の大きさを変更しますか？: ")
			(concat "arrowsize={" (YaTeX-emath-setarrow t) "}") ""))
    ...
    (progn (if (y-or-n-p "[option] 文字列を配置しますか？: ")
	   ...
	   (setq option (YaTeX-emath-option-combine "," `(,allinethickness ,Henvi ,Henvii ,arrowsize ,putstr ,putpos)))
	   (if (> (length option) 0)
	       (insert (concat "<" option ">")) "")
...
(defun YaTeX:ArrowLine ()
  (concat (YaTeX-emath-ArrowLine-option)
	  (read-string "ベクトルの始点: ")
	  (read-string "ベクトルの終点: ")))
(defun YaTeX:ArrowLines ()
  (concat (YaTeX-emath-ArrowLine-option)
	  (YaTeX-emath-tenretu-loop ";" "ベクトルの始点と終点")))
;\emPaint<#1>#2
...
(defun emath-emPaint-paintonly-option ()
  (let* ((emnum (YaTeX-emath-get-number-from-list "[option] 塗り方の指定"
						  `("白黒[濃度指定]" "カラー[色，濃度指定]"
						    "デフォルト(白黒.5)")))
	 (noudo "")
	 (color "")
	 (paint (cond ((= emnum 1) (YaTeX-emath-setoption "" "thickness" "白黒の濃度指定(.5)"))
...
(defun emath-emPaint-option (op)
  (let* ((border (if (y-or-n-p "図形の周も描画しますか？: ")
	 ...
	 (option (if (> (length border) 0)
		     (YaTeX-emath-setoption "" "bordercolor" "周の色") ""))
	 ...
	 (option (YaTeX-emath-option-combine "," `(,border ,option ,borderthickness ,paintcolor))))
    (concat option)))
(defun YaTeX:emPaint ()
...
(defun emath-emPaint*-slashonly-option ()
  (let* ((option (YaTeX-emath-setoption "" "slashcolor" "斜線の色"))
	 (angle (YaTeX-emath-tenretu-loop "," "斜線の角度[入力終了:Enter]"))
	 (angle (if (> (length angle) 0)
		    (concat "slashangle={" angle "}") ""))
	 (option (YaTeX-emath-setoption option "slashspace" "斜線の間隔"))
	 (option (YaTeX-emath-setoption option "slashthickness" "斜線の太さ(.3pt)"))
	 (option (YaTeX-emath-option-combine "," `(,option ,angle))))
    (concat option)))
(defun emath-emPaint*-option (op)
  (let* ((border (if (y-or-n-p "境界を描画しますか？: ")
		     "border" ""))
	 (option (if (> (length border) 0)
		     (YaTeX-emath-setoption "" "borderthickness" "境界の線の太さ") ""))
	 (slash (emath-emPaint*-slashonly-option))
	 (option (YaTeX-emath-option-combine "," `(,op ,border ,option ,slash))))
    (concat option)))
(defun YaTeX:emPaint* ()
  (let* ((option (emath-emPaint*-option "")))
    (insert (concat (if (> (length option) 0)
...
;    2つの区間の和集合は (,-3)|(5,) などのように，`|'を用いる
(defun YaTeX:DPkukan ()
  (let* ((kukantakasa (YaTeX-emath-setoption "" "kukantakasa" "区間の高さ[単位付き数値も可](.5)")))
    (progn (emath-emPaint-option kukantakasa)
	   (insert (concat "{" (YaTeX-emath-tenretu-loop "|" "区間の指定") "}")))))
(defun YaTeX:igenpou ()
...
(defun YaTeX:mawarikomi ()
  (let* ((emstopline (read-string "[option] 段落当初の回り込みをしない行数の指定: "))
	 ...
	 (emvec (if (y-or-n-p "[option] 図を移動させる修正ベクトルを設定しますか？")
		    (YaTeX:read-coordinates "[option] [vector] 修正ベクトル(dx,dy)(要単位): " "dx" "dy") ""))
...
(defun YaTeX:downto ()
  (let* ((emlabel-default (mapconcat 'concat
				     ...
				     ":"))
	 (emnum (YaTeX-emath-get-number-from-list "[option]"
...
(defun YaTeX:ifbox ()
  (let* ((emnuri (read-string "[option] 塗りつぶしの濃さ(0): "))
	 ...
	 (emnum (YaTeX-emath-get-number-from-list "" `("右に分岐" "左に分岐") "分岐方向の設定")))
...
(fset 'YaTeX:kurikaesi* 'YaTeX:repbox)
(defun YaTeX:jquote ()
  (let* ((emop (YaTeX-emath-setoption "" "tsep" "直前の段落との間隔調整量"))
	 (emop (YaTeX-emath-setoption emop "bsep" "直後の段落との間隔調整量"))
...
;    #3: 除式のゼロ点
(defun YaTeX:Psyndiv ()
  (let* ((option (YaTeX-emath-setoption "" "soroe" "項の揃え方[lcr](l)"))
	 (option (YaTeX-emath-setoption option "order" "余りの項の並べ方の指定")))
    (insert (concat (if (string= option "")
			"" (concat "<" option ">"))
		    "{" (YaTeX-emath-tenretu-loop "," "被除式の係数") "}"
		    "{" (read-string "除式のゼロ点: ") "}%"))))
...
;    #4 : コンマ区切り係数列
(defun YaTeX:defcsvfunc ()
  (let ((numrule (YaTeX-emath-get-number-from-list "[option] 係数列の順序" `("降べき(defalut)" "昇べき")))
	(numcsv (YaTeX-emath-get-number-from-list "[option] CSV列の取得" `("する" "しない"))))
    (insert (concat (cond ((= numrule 2) "[r]")
			  (t ""))
		    (cond ((= numcsv 1) "<csv>")
			  (t ""))
		    (read-string "定義する関数の制御綴り: ")
		    "{" (YaTeX-emath-tenretu-loop "," "各係数の設定[入力終了:Enter]") "}%"))))
;\emcancel<#1>#2
;    #1: key=val
...
(defun YaTeX:emcancel ()
  (let* ((option (YaTeX-emath-setoption "" "linethickness" "取り消し線の太さの指定"))
	 (option (YaTeX-emath-setoption option "color" "色の指定"))
	 (emnum (YaTeX-emath-get-number-from-list "[option] 取り消し線の方向指定" `("斜線[／](defalut)" "斜線[＼]" "水平線" "クロス")))
	 (setline (cond ((= emnum 2) "direction=b")
			((= emnum 3) "direction=h")
			((= emnum 4) "direction=x")
			(t "")))
	 (option (YaTeX-emath-setoption option "correct" "訂正文字列の指定"))
	 (option (YaTeX-emath-option-combine "," `(,option ,setline))))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
    ...
    (progn (while (> rowmax rownum)
	     ...
	     (while (> colmax colnum)
	       (setq colnum (+ colnum 1)
		     ...
		     colitem (YaTeX-emath-option-combine colsep `(,colitem ,item))))
	     (setq rowitem (YaTeX-emath-option-combine (concat rowsep "%\n\t") `(,rowitem ,colitem))
		   colnum 0
		   colitem ""))
	   (concat "{" rowitem "}%"))))
  ...
  (let* ((item "")
    ...
    (concat (while
		(progn
		  ...
		  (if (> (length csname) 0)
		      (progn (setq item (YaTeX-emath-setoption "" csname "定数値")
				   allitem (YaTeX-emath-option-combine ";" `(,allitem ,item))
				   item "")) nil)))
	    allitem)))
...
(defun YaTeX:pscirc ()
  (let* ((option (YaTeX-emath-drawline-option))
	 (option (YaTeX-emath-setoption option "size" "円の半径(2pt)"))
	 (option (if (> (length option) 0)
		     (concat "<" option ">") "")))
    (insert option)))
(defun YaTeX:pscirc* ()
...
(defun YaTeX:pssquare ()
  (let* ((option (YaTeX-emath-drawline-option))
	 (option (YaTeX-emath-setoption option "size" "正方形の外接円の半径"))
	 (option (if (> (length option) 0)
		     (concat "<" option ">") "")))
    (insert option)))
(defun YaTeX:pssquare* ()
...
(defun YaTeX:pstriangle ()
  (let* ((option (YaTeX-emath-drawline-option))
	 (option (YaTeX-emath-setoption option "size" "正三角形の外接円の半径"))
	 (option (if (> (length option) 0)
		     (concat "<" option ">") "")))
    (insert option)))
(defun YaTeX:pstriangle* ()
...
(defun emath-pstouhen-option ()
  (let* ((emnum (YaTeX-emath-get-number-from-list "[option] 記号の設定"
	 ...
	 (kigouopt (if (and (> emnum 0) (< emnum 8))
			   ...
			   (cond ((and (> emnum 0) (< emnum 4))
				  (concat "kigouopt={" (YaTeX-emath-drawline-option) "}"))
				 ((and (> emnum 3) (< emnum 7))
				  (concat "kigouopt={" (emath-emPaint-option "") "}"))
				 (t (concat "kigouopt={" (read-string "[option] 出力記号のオプション設定: ") "}"))) "")))
	 (option (YaTeX-emath-option-combine "," `(,kigou ,kigouopt))))
    (progn (if (> (length option) 0) ""
	     (setq option (YaTeX-emath-setoption "" "linethickness" "線の太さ")
		   option (YaTeX-emath-setoption option "kosuu" "記号の個数[1-3](1)")
		   option (YaTeX-emath-setoption option "size" "記号のサイズ(3pt)")))
	   (concat option))))
(defun YaTeX:pstouhenkigou ()
  (let* ((option (emath-pstouhen-option)))
    (insert (concat (if (> (length option) 0)
			(concat "<" option ">") "")
		    "{" (YaTeX-emath-tenretu-loop ";" "記号を配置する線分[入力終了:Enter]") "}%"))))
  ...
  (let* ((emcnt 0)
    ...
    (progn (while (> sqmax emcnt)
	     (setq item (read-string (format "%s数列の見出し指定: "
		   ...
		   midasi (YaTeX-emath-option-combine "," `(,midasi ,item))
		   emcnt (+ emcnt 1)))
	   (setq emcnt 0
		 midasi (if (> (length midasi) 0)
			    (concat "midasi={" midasi "}") "")
		 option (YaTeX-emath-option-combine "," `(,option ,midasi)))
	   (while (> sqmax emcnt)
	     (setq item (read-string (format "%s数列の指定: "
					     (if (= emcnt 0) "原"
					       (concat "第" (number-to-string emcnt) "階差"))))
		   sqs (YaTeX-emath-option-combine ";%\n\t" `(,sqs ,item))
		   emcnt (+ emcnt 1)))
	   (insert (concat (if (> (length option) 0)
			       (concat "<" option ">"))
			   "%\n\t{" sqs "}")))))
...
(defun emath-EMxymatrix-option ()
  (let* ((option (YaTeX-emath-setoption "" "wa" "矢線の横方向の長さ(3em)"))
	 (option (YaTeX-emath-setoption option "ws" "セルの横幅(1em+\\arrowcolsep)"))
	 (option (YaTeX-emath-setoption option "yul" "y軸方向の\\unitlength(2\\baselineskip)")))
    (concat option)))
